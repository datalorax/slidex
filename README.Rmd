...
## Basic usage
At present, the package exports a single function, `convert_pptx`, which takes two required argument: the `path` to the PPTX file (passed as a string), and the `author` (also passed as a sting). For example:
...
![](https://raw.githubusercontent.com/datalorax/slidex/master/docs/slidex-preview.gif)
